...
void bernoulli_gen(int v[], int n, double probability) {
  for(int i = 0; i < n; i++) {
    int r = rand_from_interval(0, RAND_MAX - 1);
    v[i] = r < probability * RAND_MAX;
  }
}
...
void pmf(double v[], int trials) {
  ...
  for(int i = 0; i < trials; i++) {
    int dsum = rand_from_interval(1, 6);
    dsum += rand_from_interval(1,6);
    v[dsum] += 1;
  }
  for(int i = 2; i <= 12; i++) {
    v[i] /= trials;
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
  for(int i = x_start; i < x_start + n; i++) {
    printf("%2d |", i);
    int mark_count = round((v[i] / y_scale));
...
void monty_hall(int n, int *p_switch_wins) {
  for(int i = 0; i < n; i++) {
    int winning_door = rand_from_interval(1, 3);
    int first_choice = rand_from_interval(1, 3);
